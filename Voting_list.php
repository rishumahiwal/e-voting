...
if (isset($_POST['submit'])) {
    ...
    $qry = "insert into election(title)values('$name')";
    mysqli_query($link, $qry);
    if (mysqli_affected_rows($link) > 0) {
        $msg = "Title Added Successfully !!!!";
    } else {
        $msg = "Error in Adding Title .....";
    }
    mysqli_close($link);
}
?>
                        ...
                        <div class="form-group">
                            <div class="row">
                                <div class="col-sm-3" align="center" style="margin-top:10px;">
                                    <div style="padding:7px;"><label for="voterid" >Title</label></div>
                                </div>

                                <div class="col-sm-9">
                                    <div style="padding:5px;">
                                        <input type="text" required name="title" class="form-control" id="vid">
                                    </div>
                                    <?php
                                    if (isset($_SESSION['utype'])) {
                                        if ($_SESSION['utype'] == 'admin') {
                                            echo "<div style='padding:5px;'>";
                                            echo "<input type='submit' name='submit' id='vid2' value='Save' class='btn btn-danger'>";
                                            echo "</div>";
                                        } else {
                                            echo "<div style='padding:5px;'>";
                    ...
                    if (isset($_SESSION['utype'])) {
                        if ($_SESSION['utype'] == 'admin') {
                            ...
                            if (mysqli_num_rows($resultset) > 0) {
                                ...
                                echo "<th>Id</th><th>Election Name</th><th></th>";
                                echo "</tr>";
                                while ($r = mysqli_fetch_array($resultset)) {
                                    echo "<tr style='color:black;'>";
                                    echo "<td>$r[0]</td>";
                                    echo "<td><a style='color:white;' href='candidate.php?id=$r[0]'>$r[1]</a></td>";
                                    echo "<td><a class='btn btn-danger' href='deleteElection.php?id=$r[0]' onclick='return confirm(\"Are you sure to delete this record?\")'>Delete Record</a></td>";
                                    echo "</tr>";
                                }
                                echo "</table>";
                                echo "</div>";
